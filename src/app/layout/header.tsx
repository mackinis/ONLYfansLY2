...
import { Menu, Radio, UserCircle, LogIn, LogOut as LogOutIcon, Loader2, Settings, User as UserIconLucide } from 'lucide-react'; 
...
export function Header() {
  ...
  useEffect(() => {
    const appUserId = (localStorage.getItem(USER_SESSION_KEY) ? JSON.parse(localStorage.getItem(USER_SESSION_KEY)!).uid : null);
    ...
    socket.on('SETTINGS_WERE_UPDATED', (data: { type: string, settings: any }) => {
      if (data.type === 'livestream') {
        console.log('Header received live stream settings update via socket.');
